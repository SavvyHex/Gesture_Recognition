...
while True:
    ...
    lmList = detector.positionFinder(image, draw=False)
    if len(lmList) != 0:
        ...
        # Hand range we gotta set this
        # Volume Range gotta find this
        #these values gonna change based on whose hand is gonna be used
    ...
    cv2.putText(image, f'{int(volPer)} %', (40, 450), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 3)


    cTime = time.time()
    fps = 1 / (cTime - pTime)
    pTime = cTime
    cv2.putText(image, f'FPS: {int(fps)}', (40, 50), cv2.FONT_HERSHEY_COMPLEX,1, (255, 255, 255), 3)
